...
class loadProperties {
    ...
    function __construct() {
        ...
        $this->error = json_decode(file_get_contents('error.json'), TRUE);
        $this->capcha = json_decode(file_get_contents('capcha.json'), TRUE);
    }
    ...
    function addLog($data, $comment) {
        // On ouvre en lecture/écriture pour créer le fichier si il n'existe pas
                fopen('resolveLog.json', "w+");
                    // Tableau contenan les informations que l'on vas encoder
                    $initJson = array(
                    $data => $comment,
                    );
                file_put_contents('resolveLog.json', json_encode($initJson));  // On encode et on écris le tableau dans le fichier JSON
    }
...
function afficheForm() {
    ...
    if ( isset($_POST['envoyer']) ) {
        ...
        //DonnéeValidée
        if ($proc->verifCapcha($data2, $model) == TRUE) {
